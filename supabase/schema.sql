...
CREATE TABLE public.task_notifications (
  ...
  type text NOT NULL CHECK (type = ANY (ARRAY['task_created'::text, 'task_assigned'::text, 'task_date_changed'::text, 'task_cancelled'::text, 'cleaning_dates_updated'::text])),
  recipient_id uuid NOT NULL,
  recipient_role text NOT NULL CHECK (recipient_role = ANY (ARRAY['owner'::text, 'manager'::text, 'cleaner'::text])),
  ...
  CONSTRAINT task_notifications_task_id_fkey FOREIGN KEY (task_id) REFERENCES public.tasks(id),
  CONSTRAINT task_notifications_recipient_id_fkey FOREIGN KEY (recipient_id) REFERENCES public.user_profiles(id)
);
CREATE TABLE public.tasks (
  ...
  CONSTRAINT tasks_calendar_entry_id_fkey FOREIGN KEY (calendar_entry_id) REFERENCES public.calendar_entries(id)
);
