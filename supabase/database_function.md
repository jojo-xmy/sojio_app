[
  ...
  {
    "schema_name": "public",
    "function_name": "backup_manage_calendar_task_association",
    "function_definition": "CREATE OR REPLACE FUNCTION public.backup_manage_calendar_task_association()\n RETURNS trigger\n LANGUAGE plpgsql\nAS $function$\r\nBEGIN\r\n  RAISE WARNING '已废弃的函数被调用';\r\n  RETURN NEW;\r\nEND;\r\n$function$\n"
  },
  {
    "schema_name": "public",
    "function_name": "generate_cleaning_tasks",
    "function_definition": "CREATE OR REPLACE FUNCTION public.generate_cleaning_tasks()\n RETURNS trigger\n LANGUAGE plpgsql\nAS $function$\nbegin\n  /*\n    当插入新的 calendar_entries 记录时，自动创建一条 draft 任务：\n    - 不再写入 tasks.description\n    - 将 owner_notes 同步到 tasks.owner_notes\n    - 使用 check_out_date 作为 tasks.check_in_date（退房日清扫）\n    - 避免重复（同 hotel_id + check_in_date）\n  */\n  insert into public.tasks (\n    hotel_name,\n    hotel_id,\n    check_in_date,\n    check_out_date,\n    guest_count,\n    status,\n    created_by,\n    owner_notes\n  )\n  select\n    h.name,\n    new.hotel_id,\n    new.check_out_date,\n    new.check_out_date,\n    new.guest_count,\n    'draft',\n    new.created_by,\n    new.owner_notes\n  from public.hotels h\n  where h.id = new.hotel_id\
  ...
  },
  {
    ...
    "function_definition": "CREATE OR REPLACE FUNCTION public.manage_calendar_task_association()\n RETURNS trigger\n LANGUAGE plpgsql\nAS $function$\nDECLARE\n    new_task_id UUID;\nBEGIN\n    IF TG_OP = 'INSERT' THEN\n        -- 创建对应的清扫任务\n        INSERT INTO public.tasks (\n            hotel_name,\n            hotel_id,\n            check_in_date,\n            check_out_date,\n            cleaning_date,\n            guest_count,\n            status,\n            created_by,\n            owner_notes\n        )\n        SELECT\n            h.name,\n            NEW.hotel_id,\n            NEW.check_in_date,  -- 修正：使用正确的入住日期\n            NEW.check_out_date,\n            NEW.check_out_date,\n            NEW.guest_count,\n            'draft',\n            NEW.created_by,\n            NEW.owner_notes\n        FROM public.hotels h\n        WHERE h.id = NEW.hotel_id\
            AND NOT EXISTS (\n                SELECT 1\n                FROM public.tasks t\n                WHERE t.hotel_id = NEW.hotel_id\
